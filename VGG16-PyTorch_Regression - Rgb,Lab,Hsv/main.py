...
parser.add_argument('--wd', '--weight-decay', default=1e-4, type=float,
                    ...
                    dest='weight_decay')
parser.add_argument('--dp', '--drop-out', default=0.5, type=float,
                    metavar='D', help='This is a dropout value for the output layer',
                    dest='drop_out')
...
parser.add_argument('-bl', '--blur', default=0, type=int,
                    metavar='BL', help='The blur kernel size to apply, if not specified, the value is false')
parser.add_argument('-ct', '--contrast', default=1.0, type=float,
                    metavar='CT', help='This sets the contrast level of the image.')
parser.add_argument('-v1', '--pack-9d-v1', action='store_true',
                    help='Pack images into 9 channels using version 1 before training')
parser.add_argument('-v2', '--pack-9d-v2', action='store_true',
                    help='Pack images into 9 channels using version 2 before training')
parser.add_argument('-si', '--show-images', action='store_true',
                    help='Only show dataset images and ungracefully exit.')
parser.add_argument('-rp', '--randomise-pixels', action='store_true',
                    help='Apply pixel randomisation')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    ********************************************"""
    if args.pretrained:
        ...
        model = VGG(vgg16_bn(True),dropout=args.drop_out)
    ...
    ********************************************"""
    ...
    for name, para in model.named_parameters():
        counter+=1
        if counter < 27: #Input layers are not allowed to train
    ...
    ********************************************"""
    ...
    train_dataset = RegrDataset(
        ...
        randomise_pixels=args.randomise_pixels,
        pack_9d={
            'v1':args.pack_9d_v1,
            'v2':args.pack_9d_v2
        })
...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    for i, (input, target) in enumerate(train_loader):#batches and targets
        ...
        acc1, acc5, _, _ = accuracy(output, target, args, 2, topk=(1, 5))
