...
class VGG(nn.Module):
    ...
    def __init__(self, features, num_classes=1, init_weights=True,**kwargs):
        super(VGG, self).__init__()

        if 'dropout' in kwargs.keys():
            print(f"Applying a dropout of {kwargs['dropout']}")
        
        self.features = features
        self.classifier = nn.Sequential(
            nn.Linear(512 * 7 * 7, 1024),
            nn.Sigmoid(),
            nn.Dropout(kwargs['dropout'] if 'dropout' in kwargs.keys() else 0.5),
            nn.Linear(1024, 512),
            nn.Sigmoid(),
            nn.Dropout(kwargs['dropout'] if 'dropout' in kwargs.keys() else 0.5),
